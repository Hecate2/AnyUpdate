...
from tests.config import short_contract_hash
...
with open('../AnyContract.manifest.json', 'r') as f:
    ...
    manifest_dict['name'] = 'AnyUpdateShort'
    manifest = json.dumps(manifest_dict, separators=(',', ':'))

client = TestClient(target_url, short_contract_hash, wallet_hash, wallet_address, 'testnet.json', '1')
client.openwallet()
...
client.invokefunction('anyUpdate', params=[nef_file, manifest, 'hello1', [1]])
client.print_previous_result()
sleep_for_next_block()
