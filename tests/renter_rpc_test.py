...
with open('../AnyUpdateRenter.nef', 'rb') as f:
    nef_file_renter = f.read()
with open('../AnyUpdateRenter.manifest.json', 'r') as f:
    manifest_dict = json.loads(f.read())
    manifest_dict['name'] = 'AnyUpdateRenter'
    manifest_renter = json.dumps(manifest_dict, separators=(',', ':'))
...
client.openwallet()
# client.invokefunction('update', params=[nef_file_renter, manifest_renter, None])
...
client.invokefunction('readContractAddress', params=[], signers=[signer], relay=False)
client.print_previous_result()
client.invokefunction('readContractExpire', params=[], signers=[signer], relay=False)
client.print_previous_result()
client.invokefunction('requestRental', params=[wallet_hash, wallet_hash, 100000, [2,1]], signers=[signer], relay=False)
client.print_previous_result()
